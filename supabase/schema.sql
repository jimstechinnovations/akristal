...
CREATE TABLE IF NOT EXISTS public.projects (
  ...
  type project_type, -- Bungalow, Duplex, Terraces, Town House, Apartment, High rising, Condominiums, Commercial Spaces
...
-- Triggers to auto-update updated_at for projects
...
-- Sellers, agents, and admins can create updates
CREATE POLICY "Sellers, agents, and admins can create updates"
  ON public.project_updates FOR INSERT
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM public.profiles
      WHERE profiles.id = auth.uid() AND profiles.role IN ('seller', 'agent', 'admin')
    ) AND
    EXISTS (
      SELECT 1 FROM public.projects
      WHERE projects.id = project_updates.project_id AND
...
-- Sellers, agents, and admins can create offers
CREATE POLICY "Sellers, agents, and admins can create offers"
...
-- Sellers, agents, and admins can create events
CREATE POLICY "Sellers, agents, and admins can create events"
